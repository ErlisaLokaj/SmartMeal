...
from domain.mappers import ShoppingMapper
...
def create_shopping_list(
    request: ShoppingListCreate, db: Session = Depends(get_db_session)
):
    """
    ...
    try:
        shopping_list = ShoppingService.build_list(
            db=db, plan_id=request.plan_id, user_id=request.user_id
        )

        # Convert to response format
        return ShoppingMapper.to_response(shopping_list)
...
def get_shopping_list(
    ...
    shopping_list = ShoppingService.get_shopping_list(
        db=db, list_id=list_id, user_id=user_id
    )
...
def get_user_shopping_lists(
    ...
    return [ShoppingMapper.to_response(sl) for sl in shopping_lists]
